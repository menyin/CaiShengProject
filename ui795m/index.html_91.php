...
<head>
	...
	<script type="text/javascript" language="javascript" src="http://cdn.597.com/js/jquery-1.8.3.min.js"></script>
	...
	<!-- <script  type="text/javascript" language="javascript" src="http://cdn.597.com/m/js/zepto.min.js"></script>
	<script  type="text/javascript" language="javascript" src="http://cdn.597.com/m/js/topsearch.js"></script>
	<link rel="stylesheet" type="text/css" href="http://cdn.597.com/m/css/jobs3.css?v=20140325" /> -->
...
.j_searchTop {height: 44px; background-color: #fff; border-bottom: 1px solid #ddd; overflow: hidden;}
...
function goso(s){
	window.location.href="/{$_COOKIE['_cityDomain']}/zhaopin/?q="+document.getElementById(s).value+"&area="+document.getElementById("noc_diqu"+s).value+"";
}
</script>
<div class="zpwrap"  id="content">
	...
	<div class="searchbox" data-search="g">
			<form name="f1g" id="frmjobSearch" method="get" action="/{$_COOKIE['_cityDomain']}/zhaopin/" >
				<div class="finput">
					<input name="q" data="g" id="jg" type="text" value="{$_GET['q']}" size="20" maxlength="12" placeholder="职位、公司名" autocomplete="off"> <i class="cross" data-cj="g"><b></b></i> 
					<div class="noc_select"></div>
					<select onchange="goso('jg');" id="noc_diqujg" name="area">
						<option value="">地区</option>
						<!--{loop $_region $key $value}-->
							<option value="{$value['region_id']}" <!--{if $value['region_id']==$_GET['area']}-->selected<!--{/if}-->>{$value[region_name_full]}</option>
						<!--{/loop}-->
					</select>
					<input name="s" class="so" type="submit" value=" 搜 索 "></div>
			</form>
		</div>
...
<!--{/if}-->
	...
	<a href="/suggestion.html" style="text-align: center;cursor: pointer;display: inline-block;width: 40px;height: 36px; padding-top: 4px;line-height: 16px; position: fixed; right:20px; bottom:95px; background-color: rgba(0,0,0,0.5); color: #fff; -webkit-tap-highlight-color: rgba(0,0,0,0);  font-size: 12px; border-radius:4px; z-index:99999;">意见<br>反馈</a>
	<!-- ios下载到桌面 -->
