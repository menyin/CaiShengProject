...
<div class="loginPop">
	<div class="loginTopBg loginTopBg2">
		<a href="/company/resume/"><i class="icon-uniE6005"></i></a>收藏的简历
	</div>
</div>
<div class="mRcmend" style="background:none;">
		...
		<div id="pageless_container" class="mRcmendListbg mFilterdListbg">
			<ul id="content">
				<!--{loop $resumes $resume}-->
				<li>
					<a href="/company/resume/resume.html?rid={$resume[_id]}"><b>{$resume[joinOffice]} <strong class="showTime">{$resume[createTime]}</strong></b><b class="subFtit">{$resume[realname]}</b><p><span>{$resume[maxEdu]}</span><span>|</span><span>{$resume[birthday]}</span><span>|</span><span>{$resume[_workYear]}</span></p></a>
					<div class="mTitimg">
...
function load(){
	...
	$.post( "/company/resume/fav.html?format=json",
	{'p':curp},
		function(data){
			if (data != null) {
