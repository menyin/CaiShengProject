...
<head>
	...
	<style>
		...
		.mKeywordP {padding-bottom:0px;}
		a.detBtn {color: #fff;height: 30px;line-height: 30px;position: absolute;right: 5px;top: 5px;width: 70px;
		text-align: center;z-index: 3;background: #e85d4e;text-align: center;border: 1px solid #d62e1b;
		border-radius: 3px; display: inline-block; font-size: 14px; left:100%; margin-left: -90px;}
		#fuliCkb {  margin:15px 0;}
		#fuliCkb label {display: block;   text-indent: 10px;  border:1px solid #ddd; border-radius:4px; background: #f8f8f8; margin-bottom: 5px; padding-bottom: 2px;}
		#fuliCkb label.cu { background: #F28479; border-color:#F28479; }
		#fuliCkb label.cu b {color:#fff;}
		body{padding-top:0px!important;}
		.freeTip{padding:50px 10px 50px 10px; font-size: 16px; text-align: center; background: #fff;}
...
<!--{else}-->
	<!--{if $app['isNewApp']!="yes"}-->
		<div class="loginPop" id="companyMainTop">
			<div class="loginTopBg "> <a href="javascript:window.history.go(-1)"><i class="icon-svg10"></i></a> 发布职位<a href="javascript:;" class="detBtn" id="doJobAdd1"><!--{if $_jid}-->修改<!--{else}-->立即发布<!--{/if}--></a> </div>
		</div>
	...
	<div class="mKeywordP">
		...
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan"><b class="ismust">*</b>职位名称</span>
			<input type="text" placeholder="请输入职位名称" name="txtStation" id="txtStation" class="mKeywText" value="<!--{if !$copy}-->{$job['jname']}<!--{/if}-->" onblur="checkJobName();" autocomplete="off">
		</div>
		<!--职位名称 结束-->
		<!--急聘 开始-->
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan"><b class="ismust">&nbsp;</b>是否急聘</span>
			<label class="graduateClass">
				<input class="checkbox" type="checkbox" name="urgency" value="1" <!--{if $job[urgency]==1}-->checked="checked"<!--{/if}--> <!--{if $isVip==0}-->disabled="disabled"<!--{/if}-->>是否急聘 <font style="color:#a1abb3; margin-left:10px;">(提示：收费会员权限)</font>
			</label>
		</div>
		<!--急聘 结束-->
		<!--职位类别  开始-->
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan"><b class="ismust">*</b>职位类别</span>
			...
			<!--数据为空时给b加个mkNull类-->
				...
				<a href="javascript:;" class="rightPop" id="addNextJobSort" style="<!--{if $job['nextJobClass']}-->display:none;<!--{/if}-->">添加次类别</a>
				<input type="hidden" name="main_jobsort" value="{$job['mainJobClassId']}">
				<input type="hidden" name="hddJobsort" value="{$job['jobClassId']}">
		</div>
		<div class="mKeywordBg mKeywordBgc" id="nextJobSortMain" style="<!--{if !$job['nextJobClass']}-->display:none;<!--{/if}-->"> <span class="tSpan">次要类别</span>
			<div class="mkDtment" id="nextJobSortDiv">
			<!--数据为空时给b加个mkNull类-->
			<b id="next_jobsort" class="mkNull"><!--{if $job['nextJobClassId']}-->{$job['nextJobClass']}<!--{/if}--></b> </div>
			<a href="javascript:;" class="rightPop" id="deleteNextJobSort">删除</a>
		...
		<div class="mKeywordBg mKeywordBgc" id="deptDiv"> <span class="tSpan"><b class="ismust">*</b>所属部门</span>
			...
			<a class="rightPop setDepart" href="/company/job/job.html?act=unitList">设置部门</a>
		</div>
		<!--所属部门  结束-->
		...
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan"><b class="ismust">*</b>招聘人数</span>
			<input type="text" placeholder="请输入招聘人数" name="txtQuantity" class="mKeywText" value="{$job['jobNumber']}" autocomplete="off">
		</div>
		...
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan">详细地址</span>
			<input type="text" placeholder="请输入详细地址" name="txtAddInfo" class="mKeywText" value="<!--{if $job['jobAddInfo']}-->{$job['jobAddInfo']}<!--{else}-->{$com['comAddress']}<!--{/if}-->" autocomplete="off">
		</div>
		<!--薪资待遇  开始-->
		...
		<div class="mKeywordBg mKeywordBgc" style="padding-left:10px;">
			<div class="mKeycAge firstSalary" style="margin-top:5px;<!--{if $job[jobSalaryType]==1}-->display:none;<!--{/if}-->" >
				<div class="mKeycAgelf" style="width:47%">
					<input type="text" placeholder="请输入最低薪资" name="hddSalary1" class="mAgeText" value="{$hddSalary1}" autocomplete="off">
					<div>元</div>
				</div>
				<div style="width:6%; float:left; text-align:center; color:#a1abb3; line-height:35px; padding:0 0 0 0px;">-</div>
				<div class="mKeycAgelf" style="width:47%">
					<input type="text" placeholder="请输入最高薪资" name="hddSalary1End" class="mAgeText" value="{$hddSalary1End}" autocomplete="off">
					<div>元</div>
				</div>
			</div>
			<div class="mKeycAge nextSalary" style="<!--{if $job[jobSalaryType]==0}-->display:none;<!--{/if}-->">
				<div class="mKeycAgelf">
					<input type="text" placeholder="请输入最低底薪" name="hddSalary2" class="mAgeText" value="{$hddSalary2}" autocomplete="off">
					<div>元</div>
				</div>
				<div style="width:12px; float:left; color:#a1abb3; line-height:35px; padding:0 0 0 5px;">-</div>
				<div class="mKeycAgelf">
					<input type="text" placeholder="请输入最高底薪" name="hddSalary2End" class="mAgeText" value="{$hddSalary2End}" autocomplete="off">
					<div>元</div>
				</div>
			</div>
			<div class="mKeycAge nextSalary" style="<!--{if $job[jobSalaryType]==0}-->display:none;<!--{/if}-->">
				<div class="mKeycAgelf">
					<input type="text" placeholder="请输入最低平均工资" name="hddSalary3" class="mAgeText" value="{$hddSalary3}" autocomplete="off">
					<div>元</div>
				</div>
				<div style="width:12px; float:left; color:#a1abb3; line-height:35px; padding:0 0 0 5px;">-</div>
				<div class="mKeycAgelf">
					<input type="text" placeholder="请输入最高平均工资" name="hddSalary3End" class="mAgeText" value="{$hddSalary3End}" autocomplete="off">
					<div>元</div>
				</div>
			</div>
		</div>
		<!--薪资待遇 结束-->
		<!--一天工作时间 开始-->
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan">一天工作时间</span>
			<select name="hddworktime" class="mKeywSelect">
				<option value="0" <!--{if $job[worktimeLimit]==0}-->selected=""<!--{/if}-->>不限</option>
				<!--{loop $workTimeArr $l}-->
					<option value="{$l['id']}" <!--{if $job[worktimeLimit]==$l['id']}-->selected=""<!--{/if}-->>{$l['name']}</option>
				<!--{/loop}-->
			</select>
		</div>
		<!--一天工作时间 结束-->
		<!--一周工作时间 开始-->
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan">一周工作几天</span>
			<select name="hddWorkday" class="mKeywSelect">
				<option value="0" <!--{if $job[workdayLimit]==0}-->selected=""<!--{/if}-->>不限</option>
				<!--{loop $workDayArr $l}-->
					<option value="{$l['id']}" <!--{if $job[workdayLimit]==$l['id']}-->selected=""<!--{/if}-->>{$l['name']}</option>
				<!--{/loop}-->
			</select>
		</div>
		<!--一天工作时间 结束-->
		<!--福利待遇开始-->
		<div class="mKeywordBg mKeywordBgc"> <span class="tSpan"><b class="ismust">*</b>福利待遇</span>
			<div class="mKeyRadio hAuto"  id="fuliCkb">
				<label>
				...
				<label>
					...
					<b>周末双休</b> </label>
				<label>
					<input type="checkbox" name="rewardID[]" value="6" <!--{if in_array('6',$job[rewardID])}-->checked="checked"<!--{/if}-->>
					<b>单休</b>
				</label>
				<label>
					<input type="checkbox" name="rewardID[]" value="7" <!--{if in_array('7',$job[rewardID])}-->checked="checked"<!--{/if}-->>
					<b>大小周</b>
				</label>
				<label>
					<input type="checkbox" name="rewardID[]" value="8" <!--{if in_array('8',$job[rewardID])}-->checked="checked"<!--{/if}-->>
					<b>加班补助</b>
				</label>
				<label>
					<input type="checkbox" name="rewardID[]" value="9" <!--{if in_array('9',$job[rewardID])}-->checked="checked"<!--{/if}-->>
					<b>班车接送</b>
				</label>
			</div>
		</div>
		...
		<div class="mKeywordBg mKeywordBgc">
			<span class="textSpan"><b class="ismust">*</b>岗位职责</span>
			<div class="template" style="display:none"></div>
			<textarea type="text" placeholder="请输入20字以上的岗位职责" name="txtContent" class="mKeywText textArea" autocomplete="off">{$job['jobContent']}</textarea>
		</div>
		...
		<div class="mKeyRadio mKeyRadio2" style="margin-bottom:0px;">
			...
			<div class="myRadioShow" id="linkwaydiv" <!--{if $job[linkWay]=='0'}-->style="display:none;"<!--{/if}-->>
				...
				<label><input type="radio" name="newLinkway" id="userNewLinkWay" value="1" <!--{if $job[linkWay]!='1'}-->checked="checked"<!--{/if}-->/>
				<b>使用新联系方式</b> </label>
				<!--{if is_array($linkWays)}-->
					<!--{loop $linkWays $key $linkWay}-->
						<div class="myInputTel linkManDiv" <!--{if ($job[linkWay]=='0' || $job[linkWay]=='1')}-->style="display:none;"<!--{/if}-->>
		...
		<div class="mKeySechBg"> <a href="javascript:void(0);" class="mKeySech" id="doJobAdd"><!--{if $_jid&&!$copy}-->修改<!--{else}-->立即发布<!--{/if}--></a> </div>
	</form>
	</div>
	...
	<input type="hidden" value="saveUnit" name="act">
		<div class="departMx"> <span class="dmxLf">排序</span> <span class="dmxRt">部门名称</span> </div>
			...
			<!--{if $unitList}-->
				...
				<div class="departMy">
					<div class="dmentLt">
						<input type="tel" value="{$l[cuDisplayOrder]}" name="editOrder[]">
	...
	<script>
		function checkJobName(){
			...
			$.getJSON("/api/web/company.api", { act: 'checkJobName', txtStation: txtStation,jobId:jid}, function(json) {
				if(json.status==0){
					return true;
				}else{
					alert('该职位名称已存在，请换一个！');
		...
		var job = {
			init:function(){
				...
				$("#deleteNextJobSort").click(function(){
					...
					$("input[name='hddJobsort']").val(main_jobsort);
				});

				/*联系方式*/
				$("#showLinkway").click(function(){
					$("#linkwaydiv").show();
					$("#notUserNewLinkWay").attr("checked","checked");
				});
				...
				$("#showEmail").click(function(){
					$("#emaildiv").show();
					$("#notNewEmail").attr("checked","checked");
				});
				...
				//开始发布职位
				$('#doJobAdd,#doJobAdd1').click(function(){
					var result = beforeSubmitCheck();
					if(result){
						showLoading();
						$('#doJobAdd,#doJobAdd1').submitForm({success:addJobCallBack, clearForm: false});
					}
				});
				//绑定模板内容
		...
		function beforeSubmitCheck(){
			...
			if(rd ==0){
				...
				var hddSalary2 = parseInt($("input[name='hddSalary2']").val());
				var hddSalary2End = parseInt($("input[name='hddSalary2End']").val());
				var hddSalary3 = parseInt($("input[name='hddSalary3']").val());
				var hddSalary3End = parseInt($("input[name='hddSalary3End']").val());
		...
		//var deptAlertHtml =  '<div class="departMybg">';
			...
			if(data.length >0){
				for(var i=0;i<data.length;i++){
					...
					deptAlertHtml = deptAlertHtml + '<input type="tel" value="'+data[i].cuDisplayOrder+'" name="editOrder[]" />';
					deptAlertHtml = deptAlertHtml + '<input type="hidden" value="'+data[i].id+'" name="deptids[]"/>';
		...
		function refreshDept(){
			var jobDeptName = $("select[name='hddUnit']").val();
			$.getJSON('/api/web/company.api',{'act':'getUnit','cid':'{$cid}'},function(result){
				...
				if(result.status>0){
					...
					if(result.dept_data.length >0){
						deptSelectHtml ="<select name='hddUnit' class='deptValueSelect mKeywSelect'>";
		...
		function deptManager(type,tree){
			var deptHtml = '<div class="departMy">'
							+'<div class="dmentLt">'
							+'<input type="tel" value="" name="addOrder[]" />'
							+'</div>'
							+'<div class="dmentRt">'
		...
		// 福利待遇样式
		$('#fuliCkb input').change(function(){
			$(this).parent().attr('class',$(this).attr('checked') ? 'cu' : '');
		});
	</script>
<!--{/if}-->
</body>
