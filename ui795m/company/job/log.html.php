...
<div class="loginPop">
	<div class="loginTopBg loginTopBg2">
		<a href="/company/resume/"><i class="icon-uniE6005"></i></a>职位被浏览记录
	</div>
</div>
<div class="mRcmend" style="background:none;">
		...
		<div class="mNoData">
			...
			<span>该功能仅收费会员方可使用，请上电脑版购买服务</a></span>
		</div>
		<!--{else}-->
		<div id="pageless_container" class="mRcmendListbg mFilterdListbg">
			...
			<ul id="content">
				<!--{loop $resumes $resume}-->
				<li>
					<a href="/company/resume/resume.html?rid={$resume[_rid]}"><b><!--{if $resume['jname']==''||$resume['unset']==1}--><span style="color:red;">职位已删除</span><!--{else}-->{$resume[jname]}<!--{/if}--> <strong class="showTime">{$resume[_time]}</strong></b><b class="subFtit">{$resume[realname]}</b><p><span>{$resume[gender]}</span><span>|</span><span>{$resume[birthday]}</span><span>|</span><span>{$resume[_workYear]}</span></p></a>
					<div class="mTitimg">
						<img src="<!--{$resume['attachment']}-->" width="62" height="78">
					</div>
			...
			<!--{else}-->
			<div class="mNoData" style="background: #e8ecef;">
				<em class="icon-uniE61E"></em>
				<span>暂无数据</span>
			</div>
			<!--{/if}-->
...
function load(){
	...
	$.post( "/company/job/log.html?format=json",
	{'p':curp},
		function(data){
			if (data != null) {
				var datalist = eval(data);
				$.each(datalist, function (i, item) {
					info="<li><a href=\"/company/resume/resume.html?rid="+item._rid+"\">";
					info+="<b>"+item.jname+"<strong class=\"showTime\">"+item._time+"</strong></b><b class=\"subFtit\">"+item.realname+"</b><p><span>"+item.gender+"</span><span>|</span><span>"+item.birthday+"</span><span>|</span><span>"+item._workYear+"</span></p></a><div class=\"mTitimg\"><img src=\""+item.attachment+"\" width=\"62\" height=\"78\"></div><em class=\"icon-uniE603\"></em></li>";
					$('#content').append(info);
				});
