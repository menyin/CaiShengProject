...
<head>
	...
	.tit strong{font-weight: bold!important;}
	.modLst .time {position: absolute; right:15px; color:#999; bottom:10px;}
	.modLst .time em {color:#19AB19;}
	</style>
</head>
<body>
<!--{template header}-->
<header class="pubtop">
	...
	<div class="cent"><p><b>我申请的职位</b></p></div>
</header>
		...
		<ul>
			...
			<li class="">
				...
				<!-- <a href="/job.html?id={$job[_jid]}"> --><i class="jpFntWes"></i>
					<p class="tit"><strong>{$job[jname]}</strong>
					<span style=" margin-left:5px;font-size:12px;">(
					<!--{if $job['status']==0}-->
						<em style="color:orange;">未查看</em>
					<!--{/if}-->
					<!--{if $job['status']==1}-->
						<em style="color:#333;">已查看</em>
					<!--{/if}-->
					<!--{if $job['status']==2}-->
						<em style="color:red;">已拒绝</em>
					<!--{/if}-->
					<!--{if $job['status']==3}-->
						<em style="color:green">同意面试</em>
					<!--{/if}-->					
					<!--{if $job['status']==9}-->
						<em style="color:gray;">已取消申请</em>
					<!--{/if}--></span>)
					</p><p>{$job[cname]}</p>
					<p>{$job[jobSalary]}<em>|</em>{$job[region_name_short]}<em></em><span class="time">{$job[createTime]}</span></p>
				</a>
			</li>
			<!--{/loop}-->
		</ul>
