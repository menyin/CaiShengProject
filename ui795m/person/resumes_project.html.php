...
<body>
	...
	<!--{template header}-->
	<header class="pubtop">
		<a href="/person/resumes.html" class="back jpFntWes">&#xf053;</a><a href="/person/resumes.html?act=project_edit" id="btnAdd1" class="detBtn">添加</a>
		<div class="cent"><p><b>项目经验</h1></b></div>
	</header>
	<!--{/if}-->
	<section class="layout">
		<div class="part modLst workEx">
			...
			<!--{if $resumeInfo[projectListInfo]}-->
				...
				<li>
					<a href="resumes.html?act=project_edit&projectid={$l[projectid]}">
						<i class="jpFntWes">&#xf105;</i>
						<p class="tit">{$l[projectName]}</p>
						<p>担任：{$l[projectDuty]}</p>
						<p>{$l[projectStart]}<em>-</em><!--{if $l[projectEnd]}-->{$l[projectEnd]}<!--{else}-->至今<!--{/if}--></p>
					</a>
				</li>
				<!--{/loop}-->
			<!--{/if}-->
		...
		<div class="btns"><a href='/person/resumes.html?act=project_edit' id="btnAdd2" class="btn3 btnsF16"><i class="jpFntWes gray">&#xf055;</i>添加项目经验</a></div>
	</section>
<!--{if $app['isNewApp']!="yes"}-->
<!--{template footer}-->
<!--{/if}-->
