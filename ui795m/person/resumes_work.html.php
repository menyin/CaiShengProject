...
<body>
	...
	<!--{template header}-->
	<header class="pubtop">
		<a href="/person/resumes.html" class="back jpFntWes">&#xf053;</a><a href="/person/resumes.html?act=work_edit" id="btnAdd1" class="detBtn">添加</a>
		<div class="cent"><p><b>工作经历</h1></b></div>
	</header>
	<!--{/if}-->
	...
	<section class="layout">
		<div class="part modLst workEx">
			...
			<!--{if $resumeInfo['workListInfo']}-->
				...
				<li>
					<a href="resumes.html?act=work_edit&rid={$rid}&workid={$l[workid]}">
						<i class="jpFntWes">&#xf105;</i>
						<p class="tit">{$l[workName]}</p>
						<p>{$l[workOffice]}</p>
						<p>{$l[workStartDate]}<em>-</em><!--{if $l[workEndDate]}-->{$l[workEndDate]}<!--{else}--> 至今<!--{/if}-->[<!--{if $l[workyear]}-->{$l[workyear]}年<!--{/if}--><!--{if $l[workmonth]}-->{$l[workmonth]}个月<!--{/if}-->]</p>
					</a>
				</li>
				<!--{/loop}-->
			<!--{/if}-->
		...
		<div class="btns"><a href='/person/resumes.html?act=work_edit' id="btnAdd2" class="btn3 btnsF16"><i class="jpFntWes gray">&#xf055;</i>添加工作经历</a></div>
	</section>
	<!--{if $app['isNewApp']!="yes"}-->
	<!--{template footer}-->
	<!--{/if}-->
