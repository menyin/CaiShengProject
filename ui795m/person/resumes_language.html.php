...
<body>
	...
	<!--{template header}-->
	<header class="pubtop">
		<a href="/person/resumes.html" class="back jpFntWes">&#xf053;</a><a href="/person/resumes.html?act=language_edit" id="btnAdd1" class="detBtn">添加</a>
		<div class="cent"><p><b>语言</h1></b></div>
	</header>
	<!--{/if}-->
	...
	<div class="part modLst langLst">
		...
		<!--{if $resumeInfo[languageListInfo]}-->
			<!--{loop $resumeInfo[languageListInfo] $l}-->
				<li><a href="/person/resumes.html?act=language_edit&languageid={$l['languageid']}"><i class="jpFntWes"></i><p><b>{$l['languageTypeInfo']}</b><em>{$l['langSkillLevelInfo']}</em></p></a></li>
			<!--{/loop}-->
		<!--{/if}-->
		</ul>
	</div>
	<div class="btns"><a href="/person/resumes.html?act=language_edit" id="btnAdd2" class="btn3 btnsF16"><i class="jpFntWes gray"></i>添加语言能力</a></div>
</section>
<!--{if $app['isNewApp']!="yes"}-->
<!--{template footer}-->
<!--{/if}-->
