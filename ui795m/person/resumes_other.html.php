...
<body>
	...
	<!--{template header}-->
	<header class="pubtop">
		<a href="/person/resumes.html" class="back jpFntWes">&#xf053;</a><a href="/person/resumes.html?act=other_edit" id="btnAdd1" class="detBtn">添加</a>
		<div class="cent"><p><b>其他</h1></b></div>
	</header>
	<!--{/if}-->
	...
	<div class="part modLst langLst">
		...
		<!--{if $resumeInfo[otherinfoListInfo]}-->
			<!--{loop $resumeInfo[otherinfoListInfo] $l}-->	
				<li><a href="/person/resumes.html?act=other_edit&otherinfoid={$l[otherinfoid]}"><i class="jpFntWes"></i><p><b>{$l[TopicDesc]}</b><em>{$l['TopicContent']}</em></p></a></li>
			<!--{/loop}-->
		<!--{/if}-->
		</ul>
	</div>
	<div class="btns"><a href="/person/resumes.html?act=other_edit" id="btnAdd2" class="btn3 btnsF16"><i class="jpFntWes gray"></i>添加其他</a></div>
</section>
<!--{if $app['isNewApp']!="yes"}-->
<!--{template footer}-->
<!--{/if}-->
