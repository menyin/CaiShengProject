var thirdSelect = {
	html: '<div class="selectParts" style="background:#fff;" id="thirdSelect">' + '<div class="selectMainTop">' + '<div class="psgSeekBg psgPrecise">' + '<span class="title"></span>' + '<a href="javascript:;" class="getBack icon-svg10" id="back"></a>' + '<a href="javascript:;" class="seekBtn" id="saveData">确定</a>' + '</div>' + '<div class="ptworkPlace" style="display:none">' + '<p>已选：</p>' + '<div id="hasSelected">' + '</div>' + '</div>' + '</div>' + '<ul class="psgMParts psgMPartsx" style="display:block; background:#fff;" id="mainData">' + '</ul>' + '</div>',
	loadinghtml: '<div id="thirdloading11"><div class="m_m_master" style="display:none;z-index:79;filter: alpha(opacity=10);opacity:0.1"></div>' + '<div class="juhua" style="display:none;z-index:80"><img src="http://cdn.597.com/m/images/m3//loadingb.gif"></div></div>',
	selectDataName: [],
	selectMax: 1,
	selectDataValue: [],
	dataCache: [],
	...
	init: function(hasSelectData, title, dataurl, param, bindValueElement, bindNameElement, callBack) {
		...
		$("#thirdSelect").stop().animate({
			width: "100%"
		}, 200);
		$("#thirdSelect .selectMainTop").stop().animate({
			width: "100%"
		}, 200);
		//去掉页面的滚动条
	},
	showLoadingOne: function(z_index_m, z_index_j) {
		...
		$("#thirdloading11 .m_m_master").show().css({
			"z-index": z_index_m
		});
		$("#thirdloading11 .juhua").show().css({
			"z-index": z_index_j
		});
	},
	...
	bindMainData: function(data, value) {
		...
		if (this.tempdata.length <= 2) {
			if (this.tempdata.indexOf(value) >= 0) {} else {
