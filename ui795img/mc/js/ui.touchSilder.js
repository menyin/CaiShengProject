...
(function(window, undefined){
	var ADSupportsTouches = ("createTouch" in document) || ('ontouchstart' in window) || 0,
		...
		ADSupportsTransition = ("WebkitTransition" in doc.style) 
							...
							|| 0,
		ADStartEvent = ADSupportsTouches ? "touchstart" : "touchstart",
		ADMoveEvent = ADSupportsTouches ? "touchmove" : "touchmove",
		ADEndEvent = ADSupportsTouches ? "touchend" : "touchend",
	...
	TouchSlider.prototype={
		...
		$E:function(classname, tagname, parent){
			var result=[],
				_array = tagname=="li" ? parent.children() : parent.getElementsByTagName(tagname);
		...
		setup: function(){
			...
			this.slides=this.opt.className?this.$E(this.opt.className,'li',this.element):this.element.children;//this.element.getElementsByTagName('li');
			...
			this.addListener(this.element,ADStartEvent,this.bind(this._start,this),true);
			this.addListener(document,ADMoveEvent,this.bind(this._move,this),true);
			this.addListener(document,ADEndEvent,this.bind(this._end,this),true);
		...
		slide:function(index, speed){
			...
			var el=this.element, timer=null,
				...
				d= (Math.abs(c)<this[size]?Math.ceil(Math.abs(c)/this[size]*speed/10):speed/10)||speed/10,//动画持续时间
		...
		_start:function(e){
			...
			if(this.opt.noclick)this.preventDefault(e);	//去掉元素默认的点击事件eg:a display:block的情况
	...
	if ( typeof define === "function") {
		define( "ui.touchSilder", [], function () { return TouchSlider; } );
	}
})(window, undefined);