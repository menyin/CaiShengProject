define(function(require,exports,module) {
	...
	var out = {
			...
			,auto : function(){
				...
				var iptTimer=null
					...
					,kwFocus = function(e,t){
						...
						if(keys[v]){return}
						$.ajax({
							url:"/search/result.php?act=searchWord&word="+encodeURIComponent(v)+"&max=5&t="+(+new Date())
							,type : "GET"
					...
					for(var i=0;i<hst.length;i++){
						...
						var li = "<li v='"+(hst.length-1-i)+"'>"+(itm.keyword.replace(/\+/g,"[$]")+"+"+(itm.jobLocIds?itm.jobLocNames:itm.workLoc)+"+"+itm.homePosition+"+"+itm.homeIndustry).replace(/^\++|\++$/g,"").replace(/\+{2,}/,"+")+"</li>";
							...
							li.click(function(){
								...
								window.location.href="/touch/search/result.uhtml?m.newSearch=true&m.searchFrom=touch&"+out.par2url(p)
							})
							ul.append($(li))
					}
			...
			,par2url : function(par){
				var tmp=""
				for(var obj in par){
					tmp+="&m."+obj+"="+encodeURIComponent(par[obj]).replace(/\+/g,"%2b")
				}
			...
			initRefreshButton: function() {//渲染点击刷新时获取到的数据		
				...
				$refresh.unbind('click').bind('click', function(event) {
					...
					if( posType === "recomTitle_recomm"){
						...
						$.ajax({
							url: '/search/recommend4Home_servlet.ujson',
			                type: 'GET',
			                dataType: 'json',
			                data:{
			                	'showPosSummary': 'detail',
	                            'page.pageNo': 1,
	                            'idsExist': '',
	                            'idsAll': cache.idsAll,
	                            '_t': Math.random()
			                ...
			                success: function(data) {
			                	if(data['rows'] && data['rows'].length > 0 && data['posId_EmPosFlag']){
                        ...
                        var data = {
                            ...
                            'p.keyword': '',
                            'p.keyword2': '',
                            'p.keywordType': param.keywordType || '',
                            'p.jobLocation': '',
                            'p.jobLocationId': param.jobLocationId || '',
                            'p.jobFunction': param.jobFunction || '',
                            'p.jobnature': param.jobnature || '15',
                            'p.degreeId1': param.degreeId1 || '',
                            'p.degreeId2': param.degreeId2 || '',
                            'p.workYear1': param.workYear1 || '-1',
                            'p.workYear2': param.workYear2 || '11',
                            'p.posPostDate': param.posPostDate || 7,
                            'p.salary': param.salary || '',
                            'p.includeNeg': param.includeNeg || '',
                            'p.inputSalary': param.inputSalary || '',
                            'p.sex': param.sex || '',
                            'p.age': param.age || '',
                            'p.sortBy': param.sortBy || 'postdate',
                            'p.totalRow': param.recordCount,
                            'p.keywordType': param.keywordType,
                            'p.otherFlag': param.otherFlag || '3',
                            'p.pageNo': 1,
                            'p.pageSize': param.pageSize || 10,
                            'p.cachePageNo': param.cachePageNo || null,
                            'p.cachePosIds': param.cachePosIds || null,
                            '_t': Math.random()
                        };
				...
				$('#homeHotJobTit li').unbind('click').bind('click', function(){
					...
					if(!$this.hasClass('actived')){//目前的选项不是目标选项 hasClass('actived')						
						...
						if(id === "recomTitle_recomm"){//智能推荐
							...
							$.ajax({
					                url: '/search/recommend4Home_servlet.ujson?page.pageSize=1&t=' + Math.random(),
					                type: 'GET',
					                dataType: 'json',	
					                beforeSend: function() {
										ui.loading.show({id: 'posRecommendLoading', z: 12});
