...
(function($) {
	...
	$.fn.dragsort = function(options) {
		...
		if (this.selector)
			$("head").append("<style type='text/css'>" + (this.selector.split(",").join(" " + opts.dragSelector + ",") + " " + opts.dragSelector) + " { cursor: move; }</style>");
		...
		this.each(function(i, cont) {
			...
			var newList = {
				...
				container: cont,
				init: function() {
					$(this.container).attr("data-listIdx", i).mousedown(this.grabItem).find(opts.dragSelector).css("cursor", "default");
				...
				grabItem: function(e) {
					if (e.which != 1 || $(e.target).is(opts.dragSelectorExclude))
						return;
					if(!$(e.target).is(opts.dragSelectExclude)&&$(e.target).closest(opts.dragSelectExclude).length <= 0) {
						return;
				    }
					var elm = e.target;
					...
					if (opts.itemSelector == "tr") {
						...
						list.placeHolderItem = list.draggedItem.clone().attr({"data-placeHolder":true,"class":"lstSdw"});
						list.draggedItem.after(list.placeHolderItem);
						list.placeHolderItem.children().each(function() { $(this).css({ borderWidth:0, width: $(this).width() + 1, height: $(this).height() + 1 }).html("&nbsp;"); });
					} else {
						list.draggedItem.after(opts.placeHolderTemplate);
						list.placeHolderItem = list.draggedItem.next().css({ height: h, width: w }).attr({"data-placeHolder":true,"class":"lstSdw"});
					}
					list.draggedItem.css({ position: "absolute", opacity: 0.1, "z-index": 999, height: h, width: w });
				...
				createDropTargets: function() {
					...
					$(lists).each(function() {
						...
						if (ph.size() > 0 && dt.size() > 0)
							...
							$(this.container).append(list.placeHolderItem.clone().removeAttr("data-placeHolder").attr({"data-dropTarget":true,"class":"lstSdw"}));
					});
				}
			};
