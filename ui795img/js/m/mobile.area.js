/**
 *   jQuery  mobile.regionId.js    职位类别
...
(function($){	
	...
	j.Default = {
		...
		itemHtml:'<dl>'+
				...
				'</dl>',	
		selectItems:null,
		allUrl:'/api/web/region.api?act=all',
		name:'hiddenName',
		onSelect:null,
		title:'请选择城市',
		isSingle:false,
	...
	j.prototype = {
		...
		initContent:function(result) {
			...
			$.each(result,function(i,n){
				...
				var isselected = arr.contains(n.regionId);
				if(isselected) {
					item.find('._selectedItem').append('全选');
				}
				item.find('._categoryName').html(n.regionName);
				item.find('._itemContainer').append(self._buildItem({regionId:n.regionId,regionName:'全选'},isselected));
				$.each(n.subItems,function(j,o){
					var iscurrselected = arr.contains(o.regionId);
					if(iscurrselected&&!isselected) {
						item.find('._selectedItem').append(o.regionName);
					}
		...
		_buildItem:function(object,ischecked) {
			if(ischecked) {
			return '<li class="check"><input type="checkbox" checked="checked" name="'+this.options.name+'" id="'+this.options.name+object.regionId+'" value="'+object.regionId+'" class="chb"><label for="'+this.options.name+object.regionName+'"><p>'+object.regionName+'</p><i class="jpFntWes">&#xf058;</i></label></li>';				
			}
			return '<li><input type="checkbox" name="'+this.options.name+'" id="'+this.options.name+object.regionId+'" value="'+object.regionId+'" class="chb"><label for="'+this.options.name+object.regionId+'"><p>'+object.regionName+'</p><i class="jpFntWes">&#xf10c;</i></label></li>';
		},
   ...
   var  o= $.fn.regionId;
   $.fn.regionId = function (option) {
     return this.each(function () {
       var $this   = $(this);
       var data    = $this.data('hb.regionId');
       var options = typeof option == 'object' && option;
       if (!data) {
 		  $this.data('hb.regionId', (data = new j(this, options)));
 	   }
   ...
   $.fn.regionId.Constructor = j;
   $.fn.regionId.noConflict = function () {
     $.fn.regionId = o;
     return this;
   };      	
