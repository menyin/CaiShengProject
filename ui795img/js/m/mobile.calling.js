			...
			if(_self.options.isSingle) {
				...
				_self._content.find('.bd ul').on('click','label',function(){
					...
					if(!parent.hasClass('check')) {
						...
						.end().find('i').html('&#xf10c;');
						var selectContent = $(this).closest('dl').find('._selectedItem').html();
						var _selectContent=selectContent.split(',');
						var t=_selectContent.indexOf(content);
						if (t>=0){
							_selectContent.splice(t, 1);
						}
						selectContent=_selectContent.join(',');
						$(this).closest('dl').find('._selectedItem').html(selectContent);
