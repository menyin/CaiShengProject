...
(function($){
	...
	d.prototype = {
		...
		initContent:function(element) {
			...
			if(this.options.isHiddenMon) {
				// 隐藏月
				this._content.find('[name="_m"]').hide();
				this._content.find('._monthtitle').hide();
			}
			// 日
			for(var k = 1;k<=31;k++) {
				if(k<10){
		...
		initEvent:function() {
			...
			if(_self.options.onChange!=null) {
				...
				var a = function() {
					...
					if(_self.options.isHiddenMon) {
						return _self._content.find('[name="_y"]').val();	
					}
