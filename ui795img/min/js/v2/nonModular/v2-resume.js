jpjs.use('@editResume, @multipleSelect, @jpCommon, @jobDater, @areaSimple, @jobsort, @form, @calling, @areaMulitiple, @jobCertificate, @jobSkill, @uploadify, @dateFormat, @confirmBox', function (m) {
    ...
    var rightSide = $('#resume_rightSide'),//侧边栏
     ...
     * @returns {number}
     * @example
     *  var myTime=getItemTime("1997/01/01");
     */
    ...
    /*为input和textarea控件添加水印功能，相当于placeholder
     使用时直接在dom上添加watermark="提示文字"*/
    $('input, textarea').watermark();

    /*顶部简历名称横幅的重命名按钮 begin*/
    var jlName = $("#jlName");
    if (jlName.length) {
        var resumeNameRules = {
                txtResumeName: {
                ...
                phoneCoder = {
                    ...
                    send: function () {
                        ...
                        $.getJSON(sendmsg_url, data, function (result) {
                            ...
                            btnSendValidate.html('<i>60</i>秒重新获取');
                            txtValidateLabel.removeClass(baseInfoEditor.allClass('label')).
    ...
    baseInfoEditor.resetData = function () {
        ...
        if ((attr = this.getDom('#spnBasicPolitical').attr('v')) != undefined) {
            if (attr) {
                dropPolitical.setSelectedValue(attr);
            } else {
        ...
        if (attr = this.getDom('#spnBasicAddress').attr('v')) {
            this.getElement('txtAddress').val(attr).resetWatermark();
        } else {
            this.getElement('txtAddress').val('');
        }
    ...
    baseInfoEditor.updatePreview = function (e) {
        ...
        var txtAddress = this.getElement('txtAddress').val();
        this.getDom('#spnBasicAddress').attr('v', txtAddress).css('display', txtAddress ? '' : 'none').text('地址：' + txtAddress || noField);
