	...
	var chkBase, sexNameBoxer, workBoxer,
		...
		var disabledClass = 'isDisabled',
			...
			remoteErrorMsg = '<em></em><i></i>该号码已被其他账号使用',
			...
			phoneCoder = {
				...
				disabled: function(){
					...
					if(txtMobilePhone.val()){
						btnMobile.addClass(disabledClass).html('更改号码');
					}else{
				...
				send:function(){
					...
					var self = this,
						data = {_txtMobile:txtMobilePhone.val()};
	...
	var resumeObjs = {
		...
		scrollPos = [
			'#baseinfor', '#jobInfor', '#appraise', '#workInfor', '#eduInfor', '#projectInfor',
	...
	rightSideBox.on('click', 'a', function(e){
		scrollToView(rightSideItem.index($(e.currentTarget)));
	});
