define(function(require,exports,module){
	...
	iosSelectUtil = {
		...
		attrToData: function(dom, index) {
			...
			for (var p in dom.dataset) {
				//if(dom.dataset.hasOwnProperty(p))
					obj[p] = dom.dataset[p];
			}
			obj['dom'] = dom;
			obj['atindex'] = index;
	...
	Layer.prototype = {
		init: function() {
			...
			document.body.appendChild(this.layer_el);
		...
		bindEvent: function() {
			var sureDom = this.layer_el.querySelectorAll('.sure');
			var closeDom = this.layer_el.querySelectorAll('.close');
			var self = this;
		...
		close: function() {
			...
			if (this.layer_el) {
				this.layer_el.parentNode.removeChild(this.layer_el);
				this.layer_el = null;
			}
		}
	}
	/*
	 ...
	 options:
	     ...
	     //(以下属性是因开始工作时间的选项而增加的属性add by cynthia)
	     dataNotChange :第一列和第二列通过parentId关联,true时数据不改变至首个data,即仅有逻辑关联(第一列选有无工作经验时,第二列和第三列(年月)数据不改变,不会跳至第一个数据)
	     scrollAction :默认为true,用于表示(仅为第二和第三)列的scroll状态,若scroll是disable的话,初始化数据时样式发生改变(仅在初始化样式数据时改变样式,需要在禁止scroll的情况下生效)
	     beforeInit:初始化数据前操作
	     afterInit:数据已初始化完毕的操作
	...
	function IosSelect(level, data, options) {
		...
		if(options.scrollAction == undefined) options.scrollAction = true;
		this.data = data;
		this.level = level || 1;
		...
		this.options.beforeInit && this.options.beforeInit(this);
		this.init();
	...
	IosSelect.prototype = {
		...
		initLayer: function() {
			...
			document.querySelector('.layer').style.height = this.itemHeight * 7 + this.headerHeight + 'px';
			...
			this.scrollOne.on('scrollEnd', function() {
				...
				if (self.level > 1 && self.options.oneTwoRelation === 1 && self.options.dataNotChange == true){
					self.setTwoLevel(self.selectOneObj.id,self.selectTwoObj.id,self.selectThreeObj.id);
				}
				else if (self.level > 1 && self.options.oneTwoRelation === 1) {
			...
			this.closeBtnDom = document.querySelector('.close');
			this.closeBtnDom.addEventListener('click', function(e) {
				window.scrollTo(0, self.offsetTop);
				document.body.style.overflow = 'auto';
			});

			this.selectBtnDom = document.querySelector('.sure');
			...
			//点击阴影部分,关闭控件
			this.hideDom = document.querySelector('.ios-select-widget-box')
			this.hideDom.addEventListener('click', function(e) {				
		...
		setOneLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			if(atIndex == 0) {
				//added by cynthia 如果找不到值,默认为第一个值,setTowLevel和setThreeLevel也添加类似的代码
				atIndex = 4;
				this.oneLevelContainDom.querySelector('li:nth-child(' + (atIndex) + ')').classList.add('at');
			}
		...
		setTwoLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			twoLevelData.forEach(function(v, i, o) {
				if (v.id === twoLevelId) {
					twoHtml += '<li ' + iosSelectUtil.attrToHtml(v) + (self.options.scrollAction?' class="at">':'>') + v.value + '</li>';
					atIndex = i + num;
				} else {
			...
			if(atIndex == 0) {
				atIndex = 4;
				this.twoLevelContainDom.querySelector('li:nth-child(' + (atIndex) + ')').classList.add('at');
			}
			var cdom = self.twoLevelContainDom.querySelector('li:nth-child(' + atIndex + ')');
			if(self.options.scrollAction){
				cdom.classList.add('at');
		...
		setThreeLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			threeLevelData.forEach(function(v, i, o) {
				if (v.id === threeLevelId) {
					threeHtml += '<li ' + iosSelectUtil.attrToHtml(v) + (self.options.scrollAction?' class="at">':'>') + v.value + '</li>';
					atIndex = i + num ;
				} else {
			...
			if(atIndex == 0) {
				atIndex = 4;
				this.threeLevelContainDom.querySelector('li:nth-child(' + (atIndex) + ')').classList.add('at');
			}
