define(function(require,exports,module) {
	...
	var out = {
			...
			var delBox = new mBox($msg,{
				...
				,onok : function(){
					$.ajax({
						...
						,data: par.data
						,beforeSend : function(){
							ui.loading.show({id:'update_loading', z:888});
							ui.mask.show({id:'update_mask', z:666});
						}
						,error : function(){
							par.error()
							ui.loading.hide({id:'update_loading'});
						...
						,success : function(data, status, xhr){
							...
							if(DATA.status==1 || DATA.success=='success'){
								var dt = util.toJSON(sessionStorage[par.myCache.par.dataKey])
									dt[par.moudle] = dt[par.moudle].filter(function(v){ return v.id!=par.moudleId })
		...
		,dic : {
			note : function(){
				var dt={},cn={},en={}
					cn["endDate"]="至今";en["endDate"]="Present";
					dt.cn = cn
					dt.en = en
				return dt
			}()
		...
		,validateData : function(ajaxData) {
			var foundErr = false
//			foundErr = !util.check(ajaxData["relationPhone"],{
//				notEmpty: false
//				,min: 7
//				,max: 15
//				,regEx : util.getRegExStr("safetel")
//				,error : function(){
//					alert("输入正确的固定电话，只能输入数字和-");
//				}
//			})
//			if(foundErr) {
//				$("#phone").focus();
...
//					,zipCode:$("#zipcode").val()
			...
			//if(!out.validateData(obj)) return;
			if(!obj.mobileNum) return alert('您未绑定手机号码，请先绑定！');
		...
		,refreshResume : function(){
			...
			$('#switch_icon').click(function(){
				...
				$label.toggleClass('switch_off').toggleClass('switch_on');
