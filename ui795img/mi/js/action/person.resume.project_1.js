define(function(require,exports,module) {
	...
	var out = {
			...
			if(jsonData.length>0){
				...
				for(var i=0;i<jsonData.length;i++){
					var dt= $.extend({},jsonData[i])
						dt.projectEndTime = dt.projectEndTime || resume.dic.note[p.lang]["endDate"]
		...
		,save : function(p){
			...
			var projectName = $("#projectName").val().replace(/\s/g, '');
				foundErr = !util.check(projectName,{
					notEmpty : true
					,max : 100
					,regex: "^[^\<\>]+$"
					,error : function(){
						message(p.lang,{cn:"请输入正确的项目名称",en:"projectName null"})
					}
				})
				if(foundErr) return;
			...
			var PostData = [{
				...
				,"projectStartTime": beginDate_y+"-"+beginDate_m
			...
			var workLen = p.data.project.length
			p.data.project = workLen == 0 ? PostData : p.data.project
			...
			var par = {
				...
				,finished : true
				,data : {
					//"project" : util.toString(PostData),
				...
				,success : function(data, status, xhr){
					...
					if(DATA.status==1){
						...
						myCache.delBtn.show()
						out.goBack(true)
						PostData[0].projectName = projectName;
						PostData[0].projectDuty = projectDuty;
						PostData[0].projectIntr = projectIntr;
						PostData[0].projectExperience = projectExperience;
						PostData[0].projectStartTime = beginDate_y+"-"+beginDate_m;
						PostData[0].projectEndTime = endDate;
						...
						myCache.info.hide()
						message(p.lang,{cn:"项目经验已保存!",en:"项目经验已保存!"})
		...
		,edit : function(){
			...
			var MODID = this.id.substring(1)
			console.log(MODID);
			var MODDATA = myCache.par.data.project.filter(function(v){ return v.id == MODID})[0]
			out.empty().show().find("button").attr("mod",MODID);	//-1标示新增
			var o = [],v = []
				o[1]="projectName";v[1]=MODDATA.projectName
				o[2]="projectDuty";v[2]=MODDATA.projectDuty
				o[3]="jobFunctionIDVal";v[3]=MODDATA.jobFunctionID
				o[4]="projectIntr";v[4]=MODDATA.projectIntr
				o[5]="beginDate_y";v[5]=MODDATA.projectStartTime.substring(0,4)
				o[6]="beginDate_m";v[6]=MODDATA.projectStartTime.substring(5,7)
				if(MODDATA.projectEndTime!=""){
					o[7]="endDate_y";v[7]=MODDATA.projectEndTime.substring(0,4)
					o[8]="endDate_m";v[8]=MODDATA.projectEndTime.substring(5,7)
				}
				o[9]="projectExperience";v[9]=MODDATA.projectExperience
