define(function(require,exports,module) {
	...
	var out = {
		...
		,edit : function(id,lang){
				myCache.listBox.hide()
				myCache.delBtn.hide()
				out.goBack(false,{title:'技能特长',ok:function(box){
					box.close();
				...
				var $delete = $('.delete')
				if(id == "null"){
					MODDATA = {SkillName:"",SkillLevel:"",levelName:"",id:""}
					out.empty().show().find("button").attr("mod",-1);	//-1标示新增
					$delete.hide();
					$('.btn1_submit').css('width','100%');
				} else{
					MODDATA = myCache.par.data.ability.filter(function(v){ return v.id == MODID})[0]
					out.empty().show().find("button").attr("mod",MODID);	//-1标示新增
					$delete.click(out.del);
				}
				var o = [],v = [],attrName = [],attrVals = []
					// o[0]="SkillName";v[0]=MODDATA.SkillName
					// util.setMultiVal(o,v)
					o[3]="level";v[3]=MODDATA.SkillLevel
					v[3]=person.dic.level[lang][MODDATA.SkillLevel], o[3]="level"
					attrName[3]="data-su_id",attrVals[3]=MODDATA.SkillLevel.replace(/^0$/,'')+"";

					myCache.recordData['level']=MODDATA.SkillLevel;
		...
		,save : function(p){
			...
			foundErr = !util.check(SkillName,{
				...
				,error : function(a,b,c){
					message(p.lang,{cn:"技能名称：不能为空,长度不能超过200字!",en:"Major:Not empty,can't be more than 200 characters!"})
				}
			})
			if(foundErr) return;
			...
			var eduLen = p.data.ability.length
			p.data.ability = eduLen == 0 ? PostData : p.data.ability
			...
			var par = {
				...
				,success : function(data){
					...
					if(DATA.success){
						...
						out.goBack(true)
						if(!PostData[0].id){
							//PostData[0].id = data.ability[0].id
