	...
	<div id="contentBody" style="visibility: visible;">
		...
		<div class="content" style="">
			<div id="main" class="security-groups" style="">
				<div class="title">
					<div class="m_bg">管理员管理 > 【{$adminLowerIds[adminUsername]}】下级列表</div>
				</div>
				<div class="quickbar">
										...
										<div style="float:left;">每页
											<select onchange="window.location.href=$(this).val();">
												<!--{loop $__ppStr $key $value}-->
													<option value="admin.html?act=nextlist&admin_id={$admin_id}{$value}" <!--{if $pp==$key}-->selected<!--{/if}-->>{$key}</option>
												<!--{/loop}-->
				...
				<div class="mainContent" style="">
					...
					<input type="hidden" name="admin_id" value="{$admin_id}">
					<input type="hidden" id="act" name="act" value="removeM">
					<div class="main_content">
						<div class="cell_tb_list">
							<table class="has_checkbox">
								...
								<!--{if $adminLowerIds[adminLowerIds]}-->
								<!--{loop $admins $admin}-->
								<tbody data-bind="foreach: items">
									<tr class="">
										...
										<td>
											<a class="btn_s" href="admin.html?act=to&nextAdminId={$admin[adminid]}">进入该用户中心</a>
											<a class="btn_s" onclick="del('{$admin_id}','{$admin[adminid]}')">移除</a>
										</td>
									</tr>
								</tbody>
								<!--{/loop}-->
							...
							<div style=" background:#efefef; height:36px; border-bottom:1px solid #ccc;">
								<input type="checkbox" name="checkall" value="on" id="selAll" onClick="javascript:SelectAll(this);" style="margin: 12px 3px 0 17px;">全选 <button type="button" onclick="submitCheck('removeM');">确定</button>
							</div>
							<!--{/if}-->
						</div>
					</div>
...
//单个删除
function del(admin_id,nextAdminId){
	...
	if(confirm('确认要移除吗？')){
		$.post('admin.html',{act:'remove',admin_id:admin_id,nextAdminId:nextAdminId},function(data){
			if(data.status<1){
