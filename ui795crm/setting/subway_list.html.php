	...
	<div id="contentBody" style="visibility: visible;">
		...
		<div class="content" style="">
			<div id="main" class="security-groups" style="">
				<div class="title">
					<div class="m_bg">站点管理 > 地铁站点设置</div>
				</div>
				<div class="quickbar">
						...
						<div class="location">
							<div class="location_main item">
								<ul>
									...
									<li>模糊查询：<input type="text" name="query" id="query" class="search input_txt" placeholder="地铁线/地铁站全名"></li>
									<li class="ml_10">
										...
										<div style="float:left;">每页
											<select onchange="window.location.href=$(this).val();">
												<!--{loop $__ppStr $key $value}-->
													<option value="subway.html?act=list{$value}" <!--{if $pp==$key}-->selected<!--{/if}-->>{$key}</option>
												<!--{/loop}-->
							...
							<table class="has_checkbox">
								...
								<tbody data-bind="foreach: items">
									<tr class="">
										...
										<td>{$subway[region_name_full]}</td>
										<td>{$subway[subway_id]}</td>
										<td>{$subway[subway_name_short]}</td>
										<td>{$subway[subway_name_full]}</td>
										<td>
											<!--{if $subway[parent_id]==0}-->
												<a class="btn_s" href="subway.html?act=roadedit&subway_id={$subway[subway_id]}">修改资料</a>
												<a class="btn_s" href="subway.html?act=edit&parent_id={$subway[subway_id]}&cityid={$subway[cityid]}">新增地铁站</a>
												<a class="btn_s" href="subway.html?act=nextlist&parent_id={$subway[subway_id]}&cityid={$subway[cityid]}">站点列表</a>
											<!--{else}-->
											<a class="btn_s" href="subway.html?act=edit&subway_id={$subway[subway_id]}">修改资料</a>
											<!--{/if}-->
