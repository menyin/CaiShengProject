		...
		<div class="wrapper">
			...
			<ul class="navs blockLink">
				<!--{if in_array('站点管理', $__r) || in_array('产品管理', $__r) || in_array('友链管理', $__r) || in_array('公告管理', $__r) || in_array('权限管理', $__r) || in_array('地铁管理', $__r) || in_array('地标管理', $__r) || in_array('下级分配', $__r) || in_array('我的下级', $__r)}-->
					<li class="items"><a class="current" href="/setting/">系统设置</a></li>
				<!--{/if}-->
				...
				<!--{/if}-->
				<!--{if $_SESSION['adminUsername']=='su' || $_SESSION['adminUsername']=='eve'}-->
					<li class="items"><a href="/company/companyM.html?act=listM&ordertype=1&query=">批量改单</a></li>
				<!--{/if}-->
							...
							<div class="m_m">
								...
								<div class="m_c noHideBlockTextLink">
									<div class="menu menu_support">
										<a onclick="Boxy.load('admin.html?act=view&admin_id={$_SESSION[adminid]}',{title: '个人信息'});">个人信息</a>
									</div>
									<div class="menu">
										<a onclick="Boxy.load('admin.html?act=password&admin_id={$_SESSION[adminid]}',{title: '密码修改'});">密码修改</a>
									</div>
