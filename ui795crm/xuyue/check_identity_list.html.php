							...
							<table class="has_checkbox">
								...
								<tbody>
									<tr class="">
										...
										<td><a href="http://www1.597.com/company/readResumes.html?id={$identity[uid]}" target="_blank">{$identity[realname]}</a></td>
										<td>{$identity[cardno]}</td>
										<td>{$identity[uploadTime]}</td>
										<td>{$identity[_isCheck]}</td>
										<td>
											<a class="btn_s" onclick="Boxy.load('check_identity.html?act=view&uid={$identity[uid]}',{title: '查看个人实名图片'});">查看</a>
											<!--{if ($identity[ischeck]==0)}-->
											<a class="btn_s" href="check_identity.html?act=check_ok&uid={$identity[uid]}">通过</a>
											<a class="btn_s" href="check_identity.html?act=check_no&uid={$identity[uid]}">屏蔽</a>
											<!--{else}-->
