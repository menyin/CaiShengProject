									...
									<tr class="<!--{if $company[_cid]==$_cid}-->cur hoverout<!--{/if}-->" style="background:<!--{if $company['vipTime1']}-->#C7E3BD;<!--{else}-->#E8AEA4;<!--{/if}-->" rel="{$company[_cid]}" cid="{$company[_cid]}">
										...
										<td>
											<div style="display:inline-block;">
												...
												-->
												<!--{if !$company['adminId']}-->
													<a class="btn_s"  onclick="joinTo('{$company[_cid]}')">跟单并登录</a>
													<a class="btn_s"  onclick="join('{$company[_cid]}');">先跟单</a>
												<!--{/if}-->
												<!--{if $company['vipTime1']}-->
...
<div id="mask" style="width: 1423px; height: 1346px; display:none;">
	<div><img src="//cdn.{ROOT_DOMAIN}/crm/images/5-121204193Q9.gif"></div>
</div>
<script type="text/javascript">
function join(cid) {
	$.getJSON('company.php',{'act':'join' ,c_id:cid}, function(data) {
		if(data.status<1){
...
function joinTo(cid) {
	$.getJSON('company.php',{'act':'joinTo' ,c_id:cid}, function(data) {
		if(data.status<1){
			$.message(data.msg, { title: "系统提示", icon: "fail" });
		}else{
			$.anchorMsg(data.msg,{icon:"success"});
			window.location.href=data.to;
			//window.open(data.to,'_blank');
		}
	});
}
