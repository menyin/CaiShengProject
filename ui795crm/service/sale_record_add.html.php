										...
										<div class="apply">
											<div class="apply_1">
												<div id="frame1" style="display:none;">
													<iframe scrolling="auto" src="/service/contract.php?act=record" width="100%" height="230" frameborder=0></iframe>
												</div>
												...
												<form id="postForm" name="postForm" method="post" action="sale_record.html">
													...
													<div class="cell_tb_list">
														<table style="margin-top: 0px;">
															...
															<tr>
																<td class="tb_title">参考合同选择：</td>
																<td colspan="3"><a onClick="contract();">导入数据</a></span></td>
															</tr>
															<tr>
																<td class="tb_title">企业名称：</td>
																<td width="592" ><span class="formText" id="myH2"><input id="c_id" type="hidden" name="c_id" value="{$result[_cid]}"><input type="text" class="text" size="50" value="{$result[cname]}"  name="cname" id="cname" /></span><span style="line-height: 30px;margin-left: 5px;"></span>
																	<span style="line-height: 30px;margin-left: 5px;"><a onClick="contract();">导入数据</a></span>
																	<div class="clear"></div></div></td>
																<td width="143" >所属站点：</td>
																<td width="469" ><span class="formText" ><input type="text" class="text" size="50" value="<!--{if $result[web_site]}-->{$result[web_site]}<!--{else}-->{$region_name_short}<!--{/if}-->" name="web_site" id="web_site" /></span><span style="line-height: 30px;margin-left: 5px;"></span>
																<div class="clear"></div></div></td>
															</tr>
															...
															<tr>
																...
																<td ><div class="formMod addressMod"><span class="formText"><input type="text" class="text" id="contract_price" name="contract_price" placeholder="" value="{$result[contract_price]}"  required="required"  onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');"/></span>
																	<span class="tipPos">
																		<span class="tipLay "></span>
																	</span>
																	<div class="clear"></div></div></td>
																<td >合同编号：</td>
																<td ><div class="formMod"><span class="formText"><input type="text" class="text" id="contractId" name="contractId" placeholder="" value="{$result[contractId]}" size="50" /></span>
																	<span class="tipPos">
																		<span class="tipLay "></span>
																	</span>
																	<div class="clear"></div></div></td>
...
<script type="text/javascript">
function sub () {
	if($('#cname').val() == ''|| typeof($('#cname').val()) == 'undefined'){
		alert('请选择或输入企业！');
		return;
	}
	...
	if($('#contract_price').val() == ''||typeof($('#contract_price').val()) == 'undefined'){
		alert('请输入付款金额！');
		return;
	}
	...
	$("#postForm").submitForm({ beforeSubmit: '', data: {}, success: function(data){
		if(data.status<1){
			...
			window.location.href='/service/sale_record.html?act=list';
		}
