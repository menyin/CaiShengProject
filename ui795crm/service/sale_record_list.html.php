							...
							<div class="location_main item">
								<ul  style="width:900px \9; *width:900px; _width:900px; ">
									<form method="get">
										...
										<li class="ml_10" <!--{if !$adminidStr}-->style="display:none;"<!--{/if}-->>跟单员：
									...
									<tr <!--{if $result1[receipt]==0}--> style="background:#F00;"<!--{/if}-->>
										...
										<!--{if (in_array('主管', $__r))}-->
											<td>
												...
												<a class="btn_s" onclick="del('{$result1[id]}','{$result1[contractId]}')">删除</a>
											</td>
										<!--{/if}-->
									</tr>
									<!--{/loop}-->
...
<script type="text/javascript">
function del(id,contractId){
	if(!id || !contractId){
		alert('参数错误');
		return;
	}
	if(confirm('确认需要删除吗？')){
		$.post('sale_record.html',{act:'del',id:id,contractId:contractId},function(data){
			if(data.status<1){
