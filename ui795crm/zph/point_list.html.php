					...
					<div class="main_content">
						<div class="cell_tb_list">
							<table class="has_checkbox">
								...
								<!--{loop $zph_points $zph_point}-->
								<tbody data-bind="foreach: items">
									<tr class="">
										<!-- <td class="td1"><input type="checkbox" class="c2_checkbox" ></td> -->
										<td>{$zph_point[region_name_full]}</td>
										<td>{$zph_point[zph_point_name]}</td>
										<td>{$zph_point[zph_point_address]}</td>
										<td>
											<a class="btn_s" href="point.html?act=edit&zph_point_id={$zph_point[zph_point_id]}">修改资料</a>
											<a class="btn_s" id="delBtn" zph_point_id="{$zph_point[zph_point_id]}" href="javascript:void(0)">删除招聘点</a>
										</td>
									</tr>
								</tbody>
								<!--{/loop}-->
...
<script type="text/javascript">
	$(".mainContent").find("tbody").find("td").find("#delBtn").click(function(){
		var zph_point_id = $(this).attr("zph_point_id");
		if(confirm('确定要删除？')){
			window.location.href = 'point.html?act=del&zph_point_id='+zph_point_id;
		}
	})
</script>
</html>