				...
				<div class="quickbar">
					...
					<div class="btns">
						<div class="btn-line clearfix">
							<a href="company.html?act=followlist2&ordertype=3&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn icon-6 <!--{if $ordertype==3}-->current<!--{/if}-->">最近登录的企业</a>
							<a href="company.html?act=followlist2&ordertype=1&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn icon-18 <!--{if $ordertype==1}-->current<!--{/if}-->">最近修改的企业</a>
							<a href="company.html?act=followlist2&ordertype=2&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn  icon-6 <!--{if $ordertype==2}-->current<!--{/if}-->">最近注册的企业</a>
							<a href="company.html?act=followlist2&ordertype=4&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn icon-6 <!--{if $ordertype==2}-->current<!--{/if}-->">最近点单的企业</a>
							<a href="company.html?act=followlist2&ordertype=5&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn icon-18 <!--{if $ordertype==5}-->current<!--{/if}-->">最近需要跟进的企业</a>
							<a href="company.html?act=followlist2&ordertype=6&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn icon-18 <!--{if $ordertype==6}-->current<!--{/if}-->">最近需要回访时间</a>
							<a href="company.html?act=followlist2&ordertype=7&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}" class="btn icon-18 <!--{if $ordertype==7}-->current<!--{/if}-->">最近跟单到期时间</a>
						</div>
					</div>
						...
						<div class="location">
							<div class="location_main item">
								...
								<!--{if $res[0]['total_found']>0}-->
									<div style="width:477px;margin:2px 10px;">
										<div style="float:left;">每页
											<select onchange="window.location.href=$(this).val();">
												<!--{loop $__ppStr $key $value}-->
													<option value="company.html?act={$act}&ordertype={$ordertype}&txtStartDate={$startDate}&txtEndDate={$endDate}&followType={$followType}&query={$q}{$value}" <!--{if $pp==$key}-->selected<!--{/if}-->>{$key}</option>
												<!--{/loop}-->
								...
								<!--{if $res[0]['total_found']>0}-->
									<div style="float:left; margin:2px 10px;">
										<div style="float:left;">每页
											<select onchange="window.location.href=$(this).val();">
												<!--{loop $__ppStr $key $value}-->
													<option value="company.html?act={$act}&ordertype={$ordertype}&txtStartDate={$startDate}&txtEndDate={$endDate}&comType={$comType}&query={$q}{$value}" <!--{if $pp==$key}-->selected<!--{/if}-->>{$key}</option>
												<!--{/loop}-->
											</select>条 共{$res[0]['total_found']}条<!--{if $res[0]['total_found']>=5000}--><font color="red">[显示前5000条]</font><!--{/if}-->
										</div>
										<div class="paginator" style="float:left;">{$showpage}</div>
