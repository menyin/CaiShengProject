				...
				<div class="quickbar">
					...
					<div class="btns">
						<div class="btn-line clearfix">
							...
							<a href="company.html?act=mylist&ordertype=7&followType={$followType}&query={$q}" class="btn icon-18 <!--{if $ordertype==7}-->current<!--{/if}-->">保留库企业</a>
						</div>
					</div>
							...
							<table class="has_checkbox">
								<thead>
									<tr class="table_header">
										...
										<th width="30px">保留库</th>
										<th width="150px">操作</th>
									</tr>
								</thead>
								<!--{loop $companys $company}-->
								<tbody>
									<tr class="" style="background:<!--{if $company['pStatus']}-->#C7E3BD;<!--{else}-->#E8AEA4;<!--{/if}-->" rel="{$company[_cid]}">
										...
										<td><!--{if $company['isSave']==1}-->是<!--{else}-->不是<!--{/if}--></td>
										<td>
										<div style="display:inline-block;">	
											...
											<a class="btn_s" href="/service/resumeCount.html?act=perjoin&c_id={$company[_cid]}" target="_blank">简历统计</a>
											...
											<!--{if $company['isSave']==1}--><a class="btn_s" id="btn_save" rel="{$company['isSave']}">取消保留库</a><!--{else}--><a class="btn_s" id="btn_save" rel="{$company['isSave']}">加入保留库</a><!--{/if}-->
											<!--4.5屏蔽，企业更新为分配方式
