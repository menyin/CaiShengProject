...
<div id="keywordContent" class="keywordContent">
	<form id="jobkeyword" action="/api/web/admin.api" method="post">
		<input type="hidden" name="act" value="tuiguang_word">
		<input type="hidden" name="id" value="{$id}">
		<p><input type="text" name="keyword" id="keyword" value="{$_GET['word']}" style="width:210px;" placeholder="请填写最多6个数字中英文组合的关键词"></p>
		...
		submit:function(){
			$("#keywordSure").click(function(){
				var keyword = $("#keyword").val();
					...
					if(keyword.length>6){
						$.anchorMsg('长度不能大于6个字符', {title:'提示', icon:'warning' });
						return false;
					}else {
