								...
								<tbody>
									<tr class="">
										...
										<td>
											...
											<!--{if (in_array('企业删除', $__r))}--><a class="btn_s" onclick="delCompany('{$company[_cid]}','ok')">确认删除</a><!--{/if}-->
											<!--{if (in_array('企业删除', $__r))}--><a class="btn_s"  onclick="delCompany('{$company[_cid]}','no')">恢复</a><!--{/if}-->
										</td>
									</tr>
								</tbody>
								<!--{/loop}-->
...
//删除
function delCompany(cid,do1){
	if(!cid){
		alert('参数错误');
		return;
	}
	if(confirm('确认要执行该操作吗？')){
		$.post('company.php',{act:'del_company',c_id:cid, do1:do1},function(data){
			if(data.status<1){
