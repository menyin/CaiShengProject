	...
	<div id="contentBody" style="visibility: visible;">
		...
		<div class="content" style="">
			<div id="main" class="security-groups" style="">
				<div class="title">
					<div class="m_bg">广告管理 > {$__advStr[$type]}  {$title}</div>
				</div>
				<div class="quickbar">
							...
							<div class="location_main item">
								<ul>
									<form method="get">
										...
										<input type="hidden" name="act1" value="{$act1}">
							...
							<table class="has_checkbox">
								...
								<tbody data-bind="foreach: items">
									<tr class="">
										...
										<td>{$adv[advid]}</td>
										<td><a href="{$adv[link_url]}" target="_blank">{$adv[cname]}</a></td>
										<td>{$__advStr[$adv[type]]}</td>
										<td>{$adv[startTime]}</td>
										<td>{$adv[endTime]}</td>
										<!-- <td><a href="adv.html?act=up&advid={$adv[advid]}&adminid={$adv[adminid]}&updateTime={$adv[updateTime]}&type={$adv[type]}&cityid={$cityid}&quyu1={$quyu1}" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('up{$adv[advid]}','','/templates/default/images/j2.png',1)"><img src="/templates/default/images/j.png" width="16" height="20" id="up{$adv[advid]}" /></a>
										...
										<td><!--{if $adv[isCheck]==1}-->√<!--{elseif $adv[isCheck]==2}-->×<!--{else}-->?<!--{/if}--></td>
										<td>
