										...
										<div class="apply">
											<div class="apply_1">
												...
												<form id="postForm" name="postForm" method="post">
                                                    ...
                                                    <input type="hidden" name="c_id" value="{$com[_cid]}">
													<div class="cell_tb_list">
														<table style="margin-top: 0px;">
															...
															<tr>
																<td class="tb_title" width="140px">分类：</td>
																<td >
																	...
																	<span style="margin-left:30px;">{$comIndustryIdStr}</span>
																</td>
															</tr>
															<tr>
...
function selectcity(){
	...
	$.get( "/area.html?act=getarea&region_id="+cityid, function(data){
		...
		var channelarr='';
		channelarr='<option value="">请选择</option>';
		$.get( "channel.html?act=get_channel&cityid="+cityid, function(json){
			var bb=eval(json);
			$.each(bb, function(i,item){
				channelarr+='<option value="'+item.id+'">'+item.channel_name+'</option>';
			});
			$('#channel_id').append(channelarr);
		});
	});
}
function selectchannel(){
	...
	var channelarr='';
	channelarr='<option value="">请选择</option>';
	$.get( "channel.html?act=get_channel&cityid="+quyu1, function(json){
		var bb=eval(json);
		$.each(bb, function(i,item){
			channelarr+='<option value="'+item.id+'">'+item.channel_name+'</option>';
		});
