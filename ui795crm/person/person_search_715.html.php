						...
						<div class="location">
							<div class="location_main item">
								<ul>
									...
									<!-- <li class="ml_10">排序：<select id="ordertype" name="ordertype" style="width:80px">
									...
									<li class="ml_10"><input type="text" name="query" id="query" class="search input_txt" value="{$q}" placeholder="用户名/邮箱/手机"></li>
									<li class="ml_10">
										...
										<div style="float:left;">每页
											<select onchange="window.location.href=$(this).val();">
												<!--{loop $__ppStr $key $value}-->
													<option value="person_search.html?act={$act}&op={$op}&query_day={$query_day}&ordertype={$ordertype}&query_type={$query_type}&query={$q}{$value}" <!--{if $pp==$key}-->selected<!--{/if}-->>{$key}</option>
												<!--{/loop}-->
							...
							<table class="has_checkbox">
								...
								<tbody>
									<tr class="">
										...
										<td>
											<a class="btn_s" href="person_search.html?act=to&r_id={$resume[_rid]}">进入</a>
											<a class="btn_s" onclick="Boxy.load('person_search.html?act=psw&r_id={$resume[_rid]}',{title: 	'企业密码重置'});">密码</a>
											<a class="btn_s" href="person_search.html?act=del&r_id={$resume[_rid]}" onclick="return confirm('确认要删除吗？');">删除</a>
										</td>
										<!-- <td>
											{if ($resume[isCheck]==0)}
