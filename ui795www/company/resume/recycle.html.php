...
<body id="body">
		...
		<section class="section">
			<hgroup>
				<div class="part part1">
					...
					<div class="recHd">
					<form id="frmApply" method="get" action="/company/resume/recycle.html" >
				...
				<!--{else}-->
					<div class="noData"><p>还没有回收简历！</p></div>
				<!--{/if}-->
				<!--{else}-->
				<div class="part part2" >
					<div class="resBd">
									...
									<table class="table">
										...
										<tbody id="lst" class="lst lst1">
											...
											<tr id="tr{$resume[id]}" class="resumeList">
												<td>
													<span class="photo"><a href="/resume.html?rid={$resume[_rid]}&join_id={$resume[_id]}" target="_blank"><!--{if $resume['attachment']}--><img src="{$resume['attachment']}"  onerror="this.onerror=null; this.src='http://cdn.597.com/img/v2/resumeM/head-defects.jpg';"><!--{/if}--></a></span>
													<span class="name"><a href="/resume.html?rid={$resume[_rid]}&join_id={$resume[_id]}" target="_blank">{$resume[realname]}</a></span>
												</td>
												<td>{$resume[gender]}</td>
												...
												<td class="time">{$resume[createTime]}</td>
												<td><!--{if $resume['status']==9||$resume['status']==10}-->求职者取消申请<!--{/if}--><!--{if $resume['status']==0}--><em class="status sta orange">未读</em><!--{/if}--><!--{if $resume['status']==1}--><em class="status sta green">已读</em><!--{/if}--><!--{if $resume['status']==2}--><em class="status sta ">已婉拒</em><!--{/if}--><!--{if $resume['status']==3}--><em class="status sta green">已邀请</em><!--{/if}--></td>
												<td>
												<a href="javascript:;" id="addNote" rel="{$resume[_id]}" resumeid="{$resume[_rid]}" class="jpFntWes <!--{if $resume[note]}-->cu<!--{/if}-->" data-placement="right" title="{$resume[note]}"></a></td>
												<td><a href="javascript:;" id="recovery" rel="{$resume[_id]}">恢复</a></td>
											</tr>
											<!--{/loop}-->
										</tbody>
									</table>
						...
						<p class="tiptxt"><strong class="orange">提示：</strong>系统只显示90天内投递的简历</p>
					</div>					
				</div>
	...
	<script>
		...
		$('#tstDropTime').droplist({
			...
			width:128,
			hddName:'apply_time',
			selectValue:'{$apply_time}',
			items:[{id:'',name:'按投递时间查看'},{id:'99',name:'今天'},{id:'1',name:'昨天'},{id:'7',name:'近7天'},{id:'30',name:'近30天'},{id:'90',name:'近90天'}],
			onSelect:function(i,name) {
			   	//选中后的事件
				apply.submit(null,0);
		}});
		$('#tstDropSta').droplist({
			...
			hddName:'status',
			selectValue:'{$_status}',
			items:[{id:'',name:'按状态查看'},{id:'99',name:'未读'},{id:'1',name:'已读'},{id:'3',name:'已邀请'},{id:'2',name:'已婉拒'},{id:'10',name:'求职者取消申请'}],
			onSelect:function(i,name) {
			   	//选中后的事件
				apply.submit(null,0);
		}});
		...
		$(document).ready(function() {
			...
			$('.resumeList').find('#addNote').click(function(){
				var resume_id=$(this).attr('resumeid');
				$.showModal('/company/resume/note.html?rid='+resume_id,{title:'备注'});
			});
			$('.resumeList').find('#recovery').click(function(){
				var join_id=$(this).attr('rel');
				$.confirm('确定恢复内容?','恢复',function(){
					$.getJSON('/api/web/company.api',{'act':'recovery' ,join_id:join_id, cidHash:{$com[cid]}}, function(json) {
						if (json.status >0){
							$.anchorMsg(json.msg);
