<?exit?>
	<div id="loginedBefore" class='tag block'>
		...
		<div class="tagC">
			<form id="personLoginForm" method="post" action="/api/web/person.api" autocomplete="off">
			...
			<form id="companyLoginForm" method="post" action="/api/web/company.api" autocomplete="off">
				<input type="hidden" name="act" value="login" />
	...
	<div id="personLoginAfter" class='personLoginAfter loginAfter none'>
		...
		<div class="loginAfterC">
			<div class="info">
				<p>您好,<span class="orange strong" id="loginPersonUsername"></span>!</p>
				<p>您目前的身份是求职者！</p>
			</div>
			<div class="btn">
				<a href="/person/resumes.html?act=base" class="loginBtn3"><span>简历管理</span></a><a href="/person/" class="loginBtn3"><span>求职中心</span></a>
			</div>
	...
	<div id="companyLoginAfter" class='companyLoginAfter loginAfter none'>
		...
		<div class="loginAfterC">
			<div class="info">
				<p>您好,<span class="orange strong" id="loginCompanyUsername"></span>!</p>
				<p>您目前的身份是企业！</p>
			</div>
			<div class="btn">
...
<script type="text/javascript" language="javascript">
	function panelLoginStatus(){
		var topUsername=readCookie('username');
		var topUserType=readCookie('userType');
		if (topUserType=='com'){
			$('#loginedBefore').hide();
			$('#companyLoginAfter').show();
			$('#personLoginAfter').hide();
			$('#loginCompanyUsername').html(topUsername);
		}
		if (topUserType=='per'){
			$('#loginedBefore').hide();
			$('#companyLoginAfter').hide();
			$('#personLoginAfter').show();
			$('#loginPersonUsername').html(topUsername);
		}
	}
		...
		login: {
			...
			_personLoginSubmit: function() {
				...
				_self.click(function() {
					...
					//表单提交
					$("#personLoginForm").submitForm({ beforeSubmit: $.proxy(personLoginForm.form, personLoginForm),success:function(result){
						if(result.status>0){
			...
			_companyLoginSubmit: function() {
				...
				_self.click(function() {
					...
					//表单提交
					$("#companyLoginForm").submitForm({ beforeSubmit: $.proxy(companyLoginForm.form, companyLoginForm),success:function(result){
						if(result.status>0){
							$.anchorMsg("登陆成功",{title: "登陆成功", icon: "success"});
